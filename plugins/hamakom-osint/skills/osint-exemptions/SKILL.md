...
description: סריקה תחקירנית של פטורים ממכרז (התקשרויות ללא מכרז) עבור דסק "המקום הכי חם בגיהנום", מעל ה-osint-db (טבלת exemptions, 137 אלף רשומות). מאתר דפוסים חשודים — ספק שחוזר שוב ושוב אצל אותו משרד או בכמה משרדים, ריכוז התקשרויות אצל מפרסם אחד, סכומים חריגים בגודלם, ונימוק דליל או גנרי שלא מצדיק פטור. הפעל כשדור כותב "תבדוק פטורים", "פטור ממכרז חשוד", "מי הספק שחוזר", "כמה X קיבל בפטור", "פטורים גדולים החודש", "תסרוק פטורים של משרד Y", "מי מקבל הכי הרבה בלי מכרז". התוצר הוא לידים לבדיקה עם עדיפות — לא ממצא מאומת. כל ליד חייב אימות מול detail_url הרשמי לפני שהוא הופך לסיפור. לתיק מאוחד על ישות חוצה-מאגר ראה osint-entity-dossier; לדייג'סט יומי של פטורים חדשים ראה osint-daily-leads.
---
...
ודא את מבנה הטבלה לפני שאילתות:

```
list_tables()                  → אישור ש-exemptions קיימת ומספר השורות
...
הרץ את הדפוסים הבאים עם `query_db`. היכן ש-`search_entity` מתאים (חיפוש ספק) הוא מהיר יותר — אבל תמיד יש גם חלופת `query_db` מלאה.

**ספק שחוזר אצל הרבה התקשרויות (כמות + סכום מצטבר):**
```sql
...
**ריכוז לפי מפרסם (איזה משרד מחלק הכי הרבה בפטור):**
```sql
...
```sql
SELECT supplier_names, COUNT(DISTINCT publisher) AS ministries,
       COUNT(*) AS deals, SUM(amount_ils) AS total_ils
FROM exemptions
WHERE supplier_names <> ''
GROUP BY supplier_names
HAVING COUNT(DISTINCT publisher) >= 3
ORDER BY ministries DESC, total_ils DESC
```
...
FROM exemptions
WHERE first_seen_at >= '2026-06-01'
ORDER BY first_seen_at DESC, amount_ils DESC
LIMIT 50
```
חלופה נוחה לפטורים שנכנסו אחרי תאריך: `new_since("exemptions", "2026-06-01")`.
...
| תקלה | פתרון |
|---|---|
| השאילתה נקטעת (>1M opcodes / 10s) | צמצם: הוסף WHERE, ORDER BY על עמודה ממוקדת, limit קטן; אל תסכם את כל הטבלה בלי תנאי |
