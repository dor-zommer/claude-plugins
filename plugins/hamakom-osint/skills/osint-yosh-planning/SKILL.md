...
```sql
SELECT pl_number, pl_name, plan_county_name, sector, pl_area_dunam,
...
SELECT pl_number, pl_name, district_name, plan_county_name, sector,
       pl_area_dunam, quantity_delta_120, depositing_date, pl_url
FROM yosh_plans
WHERE depositing_date >= '2026-05-01'
ORDER BY depositing_date DESC
LIMIT 50
```
חלופה לפי כניסה למאגר: `new_since("yosh_plans", "2026-06-01")`.
