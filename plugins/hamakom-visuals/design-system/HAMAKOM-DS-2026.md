...
החתימה של המותג — שלושת צבעי הטריו כפס אחד, **למעלה ולמטה** בכל שקף.
מימין לשמאל: **טרקוטה · מרווה · אברש** (כל אחד שליש מהרוחב).

```javascript
function SIG(f, y, h){ // h=8 בקאבר/פסקה/CTA
...
מיקום: עליון `y=0`, תחתון `y=1342`. גובה 8px.
...
- מתחת לכל זה: פס-חתימה טריקולור תחתון.
...
- gradient דיו אנכי שמתחזק למטה (alpha 0→0.1@40%→0.5@58%→0.92@78%→1@100%).
- פס-חתימה עליון. קיקר טרקוטה (`--sc-terra` על כהה). כותרת Suez One לבנה (verbatim).
- byline (`--on-dark-soft`). credit תמונה (לבן opacity .6) פינה שמאלית-תחתונה.
- פוטר: לוגו + url + פס-חתימה תחתון.

### שקף-פסקה (text)
- רקע שנהב. טקסט דיו, IBM Plex Regular, **מיושר-לעליון** (`y≈212`), x=80, w=920, RIGHT, lh 160%.
- פס-חתימה עליון. קיקר קטגוריה ("דעה" טרקוטה-עמוק) + קו טרקוטה קצר. אינדקס "02 / 09" (ink-soft, שמאל).
...
### CTA (dark)
- רקע **דיו** `#141413`. פס-חתימה עליון. וורדמרק לבן ממורכז.
