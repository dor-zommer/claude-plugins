...
# כרטיס בשליש תחתון (bottom margin 240px — מפנה מקום ל-URL), 30fps.
...
Y=$((1920-CH-240))
ffmpeg -y -v error -ss "$SS" -t "$T" -i "$IN" -i "$WORKDIR/overlay.png" -i "$CARD" \
 -filter_complex "[0:v]scale=135:240,boxblur=10:2,scale=1080:1920,eq=brightness=-0.08[bg];\
[0:v]scale=1080:1920:force_original_aspect_ratio=decrease[fg];\
[bg][fg]overlay=(W-w)/2:(H-h)/2[base];\
[base][1:v]overlay=0:0[wl];[wl][2:v]overlay=0:$Y,format=yuv420p,fps=30,setsar=1[v]" \
